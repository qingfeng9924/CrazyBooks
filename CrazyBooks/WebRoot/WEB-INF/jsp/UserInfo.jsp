...
<script>
						...
						$
								.each(
										...
										function(index, content) {
											var collection = "<tr><td><a href=book-findBookDetail?id="
													+ content.bookId
													+ ">"
													+ content.bookName
													+ "</a></td><td>"
													...
													+ "</td><td><a href='javascript:void(0)' onclick='deleteUserCollection("
													+ content.bookId
													+ ")'>删除</a></td></tr>";
											$("#myCollection tbody").append(
													collection);
										});
					}
				});
	...
	function deleteUserCollection(id) {
		$.ajax({
			type : "post",
			url : "UserdeleteUserCollectionAction",
			data : {
				bookId : id
			},
			dataType : "json",
		...
		$.ajax({
			...
			success : function(data) {
				...
				$.each(d, function(index, content) {
					var comment = "<tr><td><a href=book-findBookDetail?id="
							+ content.bookId + ">" + content.bookName
			...
			success : function(data) {
				...
				$.each(d, function(index, content) {
					...
					if(content.returnTime != undefined){
						comment = "<tr><td><a href=book-findBookDetail?id="
							+ content.bookId + ">" + content.bookName
							+ "</a></td><td>" + content.bookAuthor
							+ "</td><td>" + content.bookPublisher + "</td><td>"
							+ content.borrowTime.split(" ")[0] + "</td><td>"
							+ content.deadline.split(" ")[0] + "</td><td>"
							+ content.returnTime.split(" ")[0] + "</td><td>"
					...
					if(content.returnTime==undefined){
						var comment = "<tr><td><a href=book-findBookDetail?id="
							...
							+ "</td><td>" + content.bookPublisher + "</td><td>"
							+ content.borrowTime.split(" ")[0] + "</td><td>"
							+ content.deadline.split(" ")[0] + "</td><td><a href='UserreturnBookAction?bookId="+content.bookId+"'>"
							+ "我要还书" + "</a></td><td>"
							+ "无" + "</td></tr>";
					}
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (d.type == "success") {
					alert("充值成功!");
					$("#txt_balance").val(
							parseInt($("#txt_balance").val())
									+ parseInt($("#recharge_amount").val()));
					$("#recharge_amount").val("");
...
<body>
	...
	<div class="container" id="myDiv"
		...
		<table id="myBorrow"
			style="width:1500px;float:left;margin-left:20px;margin-top:30px;display:none
			">
			<thead>
